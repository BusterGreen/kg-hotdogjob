...
local function FinishMinigame(faults)
    ...
    if Config.Stock[Quality].Current + 1 <= Config.Stock[Quality].Max[Config.MyLevel] then
        TriggerServerEvent('kg-hotdogjob:server:UpdateReputation', Quality)
        if Config.MyLevel == 1 then
            KGCore.Functions.Notify(Lang:t('success.made_hotdog', { value = Config.Stock[Quality].Label }), 'success')
            ...
            if Luck == LuckyNumber then
                KGCore.Functions.Notify(Lang:t('success.made_luck_hotdog', { value = LuckyAmount, value2 = Config.Stock[Quality].Label }), 'success')
        ...
        KGCore.Functions.Notify(Lang:t('error.no_more', { value = Config.Stock[Quality].Label }), 'error')
    end
    PreparingFood = false
end
...
local function StartHotdogMinigame()
    PrepareAnim()
    local result = exports['kg-minigames']:KeyMinigame(10)
...
local function StartWorking()
    KGCore.Functions.TriggerCallback('kg-hotdogjob:server:HasMoney', function(HasMoney)
        if HasMoney then
            if Config.UseTarget then
                ...
                exports['kg-target']:AddTargetEntity(StandObject, {
                    options = {
                        {
                            icon = 'fas fa-hand',
                        ...
                        type = 'client',
                        event = 'kg-hotdogjob:client:ToggleSell',
                        canInteract = function()
                            return IsWorking
                        end
                    }
                ...
                UpdateUI()
                KGCore.Functions.Notify(Lang:t('success.deposit', { deposit = Config.StandDeposit }), 'success')
            else
                local SpawnCoords = Config.Locations['spawn'].coords
                IsWorking = true
                LoadModel('prop_hotdogstand_01')
            ...
            KGCore.Functions.Notify(Lang:t('error.no_money'), 'error')
        end
    end)
end
...
local function StopWorking()
    if DoesEntityExist(StandObject) then
        KGCore.Functions.TriggerCallback('kg-hotdogjob:server:BringBack', function(DidBail)
            if DidBail then
                DeleteObject(StandObject)
                ClearPedTasksImmediately(PlayerPedId())
                IsWorking = false
                ...
                KGCore.Functions.Notify(Lang:t('success.deposit_returned', { deposit = Config.StandDeposit }), 'success')
            else
                KGCore.Functions.Notify(Lang:t('error.deposit_notreturned'), 'error')
            end
        end)
    else
        KGCore.Functions.Notify(Lang:t('error.no_stand_found'), 'error')
        IsWorking = false
        StandObject = nil
        IsPushing = false
        IsUIActive = false
...
local function SellToPed(ped)
    ...
    while pedDist > OffsetData.Distance do
        ...
        if PlayerDist > 10.0 then
            ...
            KGCore.Functions.Notify(Lang:t('error.too_far'), 'error')
            break
        end
        Wait(100)
    end
            ...
            if SellingData.Hotdog ~= nil then
                ...
                if Config.UseTarget then
                    if not zoneMade then
                        zoneMade = true
                        exports['kg-target']:AddEntityZone('sellingDogPed', ped, {
                            name = 'sellingDogPed',
                            debugPoly = false,
                        }, {
                            options = {
                                {
                                    ...
                                    action = function(entity)
                                        KGCore.Functions.Notify(Lang:t('success.sold_hotdogs', { value = HotdogsForSale, value2 = (HotdogsForSale * SellingPrice) }), 'success')
                                        TriggerServerEvent('kg-hotdogjob:server:Sell', pedCoords, HotdogsForSale, SellingPrice)
                                        SellingData.HasTarget = false
                                        LoadAnim('mp_common')
                                        ...
                                        SellingData.Hotdog = nil
                                        exports['kg-target']:RemoveZone('sellingDogPed')
                                        zoneMade = false
                                    end,
                                },
                                {
                                    ...
                                    label = 'Decline offer',
                                    action = function(entity)
                                        KGCore.Functions.Notify(Lang:t('error.cust_refused'), 'error')
                                        SellingData.HasTarget = false
                                        FreezeEntityPosition(entity, false)
                                        SetPedKeepTask(entity, false)
                                        SetEntityAsNoLongerNeeded(entity)
                ...
                SellingData.HasTarget = false
                SellingData.Hotdog = nil
                KGCore.Functions.Notify(Lang:t('error.no_dogs'), 'error')
                break
            end
        else
            SellingData.HasTarget = false
...
local function ToggleSell()
    ...
    if StandObject ~= nil then
        ...
        KGCore.Functions.Notify(Lang:t('error.no_stand'), 'error')
    end
end
...
RegisterNetEvent('kg-hotdogjob:client:UpdateReputation', function(JobRep)
    PlayerData.metadata['rep'] = JobRep
    UpdateLevel()
end)

RegisterNetEvent('kg-hotdogjob:client:ToggleSell', function()
...
RegisterNetEvent('kg-hotdogjob:staff:DeletStand', function()
    local ped = PlayerPedId()
    ...
    if Object ~= nil then
        ...
        if ObjectDistance <= 5 then
            ...
            Wait(100)
            DeleteEntity(Object)
            KGCore.Functions.Notify(Lang:t('info.admin_removed', 'primary'))
        end
    end
end)
...
CreateThread(function()
    if Config.UseTarget then
        exports['kg-target']:AddBoxZone('hotdog_start', vector3(Config.Locations['take'].coords.x, Config.Locations['take'].coords.y, Config.Locations['take'].coords.z), 1, 1, {
            name = 'hotdog_start',
            debugPoly = false,
        ...
        hotdogStart:onPlayerInOut(function(isPointInside)
            if isPointInside then
                ...
                if PlayerData.job.name == 'hotdog' then
                    if not IsWorking then
                        exports['kg-core']:DrawText(Lang:t('info.start_working'), 'left')
                        CreateThread(function()
                            while inZone do
                                Wait(0)
                                if IsControlJustPressed(0, 38) then
                        ...
                        exports['kg-core']:DrawText(Lang:t('info.stop_working'), 'left')
                        CreateThread(function()
                            while inZone do
                                Wait(0)
                                if IsControlJustPressed(0, 38) then
